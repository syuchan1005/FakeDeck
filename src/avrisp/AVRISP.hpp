...
typedef enum {
    AVRISP_STATE_INIT = 0,
    AVRISP_STATE_IDLE,    // no active TCP session
...
class AVRISP {
public:
    AVRISP(uint16_t port, uint8_t reset_pin, uint32_t spi_freq=AVRISP_SPI_FREQ, bool reset_state=false, bool reset_activehigh=false);
