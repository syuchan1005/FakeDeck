...
AVRISP::AVRISP(uint16_t port, uint8_t reset_pin, uint32_t spi_freq, bool reset_state, bool reset_activehigh):
...
inline void AVRISP::_reject_incoming(void) {
    while (_server.hasClient()) _server.accept().stop();
}

uint8_t AVRISP::getch() {
...
void AVRISP::flash(uint8_t hilo, int addr, uint8_t data) {
...
void AVRISP::commit(int addr) {
    spi_transaction(0x4C, (addr >> 8) & 0xFF, addr & 0xFF, 0);
    delay(AVRISP_PTIME);
}

//#define _addr_page(x) (here & 0xFFFFE0)
int AVRISP::addr_page(int addr) {
...
void AVRISP::write_flash(int length) {
    fill(length);
...
uint8_t AVRISP::write_flash_pages(int length) {
    int x = 0;
...
uint8_t AVRISP::write_eeprom(int length) {
...
uint8_t AVRISP::write_eeprom_chunk(int start, int length) {
...
bool AVRISP::flash_read_page(int length) {
...
bool AVRISP::eeprom_read_page(int length) {
